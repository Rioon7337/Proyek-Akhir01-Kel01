    ...
    <div class="stack-area">
        ...
        @foreach($galeriByKategori as $kategori => $items)
            ...
            @foreach($items as $item)
                ...
                @php
                    // Gambar disimpan sebagai base64 data URI lengkap (data:image/...;base64,...)
                    // Gunakan langsung jika sudah berupa data URI, atau fallback ke storage path
                    if($item->gambar && str_starts_with($item->gambar, 'data:')){
                        $src = $item->gambar;
                    } elseif($item->gambar) {
                        $src = asset('storage/' . $item->gambar);
                    } else {
                        $src = 'https://via.placeholder.com/300x500?text=No+Image';
                    }
                @endphp
