...
<style>
    ...
    */
    ...
    .news-hero {
        ...
        height: 55vh;
        min-height: 360px;
       background: url('{{ asset("image/Sipinsur/sipinsur1.jpg") }}') center/cover no-repeat;
        display: flex;
        align-items: center;
    ...
    <div class="news-track" id="newsTrack">
        ...
        <div class="circle-card" onclick="openReader({{ $item->id }})">
            <div class="img-circle-frame">
                @if($item->gambar)
                    <img src="{{ $item->gambar && !str_starts_with($item->gambar, 'data:') ? asset('storage/' . $item->gambar) : $item->gambar }}" alt="{{ $item->judul }}" loading="lazy">
                @else
...
<script>
    ...
    function openReader(id) {
        ...
        var imgSrc = item.gambar || '{{ asset("image/default.jpg") }}';
        if (item.gambar && !item.gambar.startsWith('data:')) {
